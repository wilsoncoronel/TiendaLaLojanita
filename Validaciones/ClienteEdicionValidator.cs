...
{
    public class ClienteEdicionValidator : AbstractValidator<ClienteEditarDTO>
    {
        public ClienteEdicionValidator()
        {
            RuleFor(x => x.Nombres)
            ...
            RuleFor(x => x.DireccionEdicionDto.IdCiudad)
                .GreaterThan(0).WithMessage("Debe seleccionar una ciudad.");
            RuleFor(x => x.DireccionEdicionDto.Descripcion)
                .NotEmpty().WithMessage("El apellido es obligatorio.")
                .NotNull().WithMessage("El nombre no puede ser nulo.")
